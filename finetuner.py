...
[For PYTHIA 160M finetuning with 1900 tokens each]

deepspeed --num_gpus=1 minimal_trainer.py \
--deepspeed ds_config.json \
--model_name_or_path pythia-160m-deduped \
--do_train \
--do_eval \
--block_size 1900 \
--fp16 \
...
# Example json for dsconfig (ds_config.json)
...
def main():
    ...
    train_dataset = load_from_disk("dataset/train_data") # replace custom_train with your path
    eval_dataset = load_from_disk("dataset/eval_data") # replace custom eval with your path
    ...
    if training_args.do_train:
        ...
        try:
            ...
            model.save_pretrained(os.path.join(wandb.run.dir, "saved_model"))
            logger.error(str(e))
            logger.error("Training ended due to error above.")
            return
